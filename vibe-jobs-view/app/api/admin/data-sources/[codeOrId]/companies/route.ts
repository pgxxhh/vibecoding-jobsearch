...
async function forward(req: NextRequest, params: { codeOrId: string }, method: 'GET' | 'POST') {
  ...
  const upstream = buildBackendUrl(base, `/admin/data-sources/${params.codeOrId}/companies`);
...
export async function POST(req: NextRequest, context: { params: { codeOrId: string } }) {
  return forward(req, context.params, 'POST');
}