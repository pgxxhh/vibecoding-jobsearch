              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {blueprint.status && <Badge tone={statusTone(blueprint.status)}>{blueprint.status}</Badge>}
                    <Badge tone={blueprint.enabled ? 'brand' : 'default'}>{blueprint.enabled ? '已启用' : '已停用'}</Badge>
                    {blueprint.autoGenerated != null && (
                      <Badge tone={blueprint.autoGenerated ? 'brand' : 'muted'}>
                        {blueprint.autoGenerated ? '自动生成' : '手动配置'}
                      </Badge>
                    )}
                  </div>
                  <dl className="mt-3 grid gap-2 text-sm text-gray-600 sm:grid-cols-2">
                    ...
                    <div>
                      <dt className="font-medium text-gray-700">生成方式</dt>
                      <dd>
                        {blueprint.autoGenerated == null
                          ? '—'
                    ...
                    <div>
                      <dt className="font-medium text-gray-700">创建时间</dt>
                      <dd>{formatDateTime(blueprint.createdAt)}</dd>
                    </div>
                    <div>
                      <dt className="font-medium text-gray-700">更新时间</dt>
                      <dd>{formatDateTime(blueprint.updatedAt)}</dd>
                    </div>
                    <div>
                      <dt className="font-medium text-gray-700">最近生成时间</dt>
                      <dd>{formatDateTime(blueprint.generatedAt)}</dd>
                    </div>
                    <div>
                      <dt className="font-medium text-gray-700">最近生成操作者</dt>
                      <dd>{blueprint.generatedBy ?? '—'}</dd>
                    </div>
                  </dl>
                </div>
