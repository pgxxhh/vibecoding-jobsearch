...
public class MockCareersApiSource implements SourceClient {
    ...
    @Override public List<FetchedJob> fetchPage(int page, int size){
        if(page>1) return List.of();
        List<FetchedJob> list = new ArrayList<>();
        Job job = Job.builder().source(sourceName()).externalId("REQ-1001").title("Senior Backend Engineer (Java)")
        ...
        String content = "Join our " + company + " platform team and design high-throughput services.";
        list.add(new FetchedJob(job, content));
        return list;
    }
}
